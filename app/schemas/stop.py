...
class StopSchedule(BaseModel):
    trip: ScheduledTrip = Field(description="Trip that the stop time belongs to")
...
class StopDetailedResponse(BaseModel):
    ...
    latitude: float = Field(description="Latitude value of the stop")
    longitude: float = Field(description="Longitude value of the stop")
    stop_times: List[StopSchedule] = Field(description="Scheduled stop times on this stop")
